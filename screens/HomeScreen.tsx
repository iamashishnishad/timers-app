...
//                       </TouchableOpacity>
//                       <TouchableOpacity 
//                         onPress={() => setTimers(resetTimer(timer.id, timers))} 
...
//                         style={[themedStyles.button, { backgroundColor: theme === 'light' ? '#ff4444' : '#cc0000' }]}
//                       >
...
//             Selected: {formatDuration(minutes, seconds)}
//           </Text>
//           <View style={{ flexDirection: 'row', justifyContent: 'center' }}>
//             <Picker
//               selectedValue={minutes}
...
// import AsyncStorage from '@react-native-async-storage/async-storage';
// import { Picker } from '@react-native-picker/picker';
// import * as FileSystem from 'expo-file-system'; // Use Expo FileSystem
// import { Timer, HistoryEntry } from './types';
// import { createStyles } from './styles';
// import {
//   startTimer,
...
//     if (history.length === 0) {
//       Alert.alert('No Data', 'There is no timer history to export.');
//       return;
//     }
...
//       Alert.alert('Success', `Timer history exported to ${path}`);
//     } catch (error) {
...
import * as Sharing from 'expo-sharing'; // Add expo-sharing
import { Timer, HistoryEntry } from './types';
import { createStyles } from './styles';
import {
  startTimer,
...
const HomeScreen: React.FC<Props> = ({ navigation }) => {
  ...
  const exportData = async () => {
    ...
    try {
      await FileSystem.writeAsStringAsync(path, data);
      // Share the file after writing
      if (await Sharing.isAvailableAsync()) {
        await Sharing.shareAsync(path, {
          mimeType: 'application/json',
          dialogTitle: 'Save or Share Timer History',
        });
      } else {
        Alert.alert('Sharing Not Available', `File saved to ${path}, but sharing is not supported on this device.`);
      }
    } catch (error) {
      console.error('Error exporting data:', error);
      Alert.alert('Error', 'Failed to export timer history: ' + error.message);
    }
  };
